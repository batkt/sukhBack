...
// Custom liftShalgaya routes - upsert by baiguullagiinId + barilgiinId to prevent duplicates
router.get("/liftShalgaya", tokenShalgakh, async (req, res, next) => {
  try {
    ...
    if (body.barilgiinId) {
      query.barilgiinId = String(body.barilgiinId);
    }

    const khuudasniiDugaar = body.khuudasniiDugaar ? Number(body.khuudasniiDugaar) : 1;
    const khuudasniiKhemjee = body.khuudasniiKhemjee ? Number(body.khuudasniiKhemjee) : 100;

    const jagsaalt = await LiftShalgaya(tukhainBaaziinKholbolt)
      .find(query)
      .skip((khuudasniiDugaar - 1) * khuudasniiKhemjee)
      .limit(khuudasniiKhemjee);

    const niitMur = await LiftShalgaya(tukhainBaaziinKholbolt).countDocuments(query);
...
// POST/PUT - Upsert liftShalgaya by baiguullagiinId + barilgiinId
router.post("/liftShalgaya", tokenShalgakh, async (req, res, next) => {
  try {
    ...
    if (!baiguullagiinId || !barilgiinId) {
      return res.status(400).json({
        success: false,
        aldaa: "baiguullagiinId болон barilgiinId шаардлагатай",
      });
    }
    ...
    const result = await LiftShalgaya(tukhainBaaziinKholbolt).findOneAndUpdate(
      {
    ...
    console.log(`✅ [LIFTSHALGAYA] Upserted for baiguullaga: ${baiguullagiinId}, barilga: ${barilgiinId}`);
...
router.put("/liftShalgaya/:id", tokenShalgakh, async (req, res, next) => {
  try {
    const { id } = req.params;
    const { baiguullagiinId, barilgiinId, choloolugdokhDavkhar } = req.body;
    ...
    const result = await LiftShalgaya(tukhainBaaziinKholbolt).findByIdAndUpdate(
      id,
      { choloolugdokhDavkhar: choloolugdokhDavkhar || [] },
      { new: true }
    );
...
router.delete("/liftShalgaya/:id", tokenShalgakh, async (req, res, next) => {
  try {
    const { id } = req.params;
    const { baiguullagiinId } = req.body || req.query || {};
    ...
    const result = await LiftShalgaya(tukhainBaaziinKholbolt).findByIdAndDelete(id);
