...
exports.qpayNekhemjlekhGargaya = asyncHandler(async (req, res, next) => {
  try {
    const { db } = require("zevbackv2");
    const nekhemjlekhiinTuukh = require("../models/nekhemjlekhiinTuukh");
    
    const baiguullagiinId = req.body.baiguullagiinId;
    const nekhemjlekhiinId = req.body.nekhemjlekhiinId;
    ...
    if (!kholbolt) {
      throw new aldaa("Organization not found");
    }
    ...
    if (!nekhemjlekh) {
      throw new aldaa("Invoice not found");
    }

    // Check if already paid
    if (nekhemjlekh.tuluv === "Төлсөн") {
      throw new aldaa("Invoice already paid");
    }

    // Get QPay credentials
    const dans = await Dans(kholbolt).findOne({
      dugaar: nekhemjlekh.dansniiDugaar,
    });
    
    if (!dans || !dans.qpayAshiglakhEsekh || !dans.qpayUsername || !dans.qpayPassword || !dans.qpayInvoiceCode) {
      throw new aldaa("QPay configuration not found");
    }
    ...
    // Create QPay invoice object
    const qpayObject = {
      invoice_code: dans.qpayInvoiceCode,
      sender_invoice_no: nekhemjlekh.dugaalaltDugaar.toString(),
      invoice_receiver_code: dans.qpayInvoiceCode,
      invoice_description: `Нэхэмжлэх #${nekhemjlekh.dugaalaltDugaar} - ${nekhemjlekh.ner}`,
      allow_partial: false,
      minimum_amount: null,
      allow_exceed: false,
      maximum_amount: null,
      amount: nekhemjlekh.niitTulbur,
      callback_url: `${process.env.UNDSEN_SERVER}/qpayNekhemjlekhCallback/${baiguullagiinId}/${nekhemjlekhiinId}`,
    };

    // Create QPay invoice
    const khariu = await qpayShivye(token, qpayObject, next);
    
    if (khariu && khariu.invoice_id) {
      // Update nekhemjlekh with QPay invoice ID
      nekhemjlekh.qpayInvoiceId = khariu.invoice_id;
      nekhemjlekh.qpayUrl = khariu.urls.find(url => url.name === "payment_page")?.link;
      await nekhemjlekh.save();
    }

    res.send({
      success: true,
      invoice_id: khariu.invoice_id,
      qpay_url: khariu.urls?.find(url => url.name === "payment_page")?.link,
      amount: nekhemjlekh.niitTulbur,
